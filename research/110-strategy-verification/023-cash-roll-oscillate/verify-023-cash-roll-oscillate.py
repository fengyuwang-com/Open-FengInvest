#!/usr/bin/env python3
"""verify-023-cash-roll-oscillate.py — 论断 #23: 钱仓滚存震荡市优于持有

测试方法:
  1. 获取所有 US 股票的日线数据, 转换为月末收盘价
  2. 获取 SPY 的月线数据, 计算各月的 6 个月收益
  3. 筛选"震荡市"月份 (SPY 6个月收益在 -5%~+5% 之间)
  4. 钱仓滚存策略:
     - 每月末, 对所有 US 股票计算当月 (1个月) 动量
     - 按动量排序, 选前 20%
     - 等权重持有 1 个月
  5. 对比钱仓滚存策略 vs SPY 持有人在震荡市的收益率
...
OSCILLATE_LOWER = -0.05
OSCILLATE_UPPER = 0.05
...
def run_backtest():
    ...
    #   If oscillating:
    #     Stock_1m_ret = (close[M_i] - close[M_{i-1}]) / close[M_{i-1}]  -- momentum signal using M_i = all_months[i]
    #     Cash-roll holding period: all_months[i] -> all_months[i+1]

    oscillating_results = []
    all_regime_counts = defaultdict(int)  # track months per regime
    ...
    for i in range(6, len(all_months) - 1):
        ...
        # Classify regime
        if spy_6m_ret < OSCILLATE_LOWER:
            regime = "bear_or_decline"
        elif spy_6m_ret > OSCILLATE_UPPER:
            regime = "bull"
        else:
            regime = "oscillating"
        all_regime_counts[regime] += 1

        # Only process oscillating months
        if regime != "oscillating":
            continue

        # Compute 1-month momentum for each stock
        stock_momentum = []  # list of (idx_id, momentum_return)
    ...
    if not oscillating_results:
        print("WARNING: No oscillating months found")
        return {"error": "no oscillating months found", "total_spy_months": len(all_months)}

    # Aggregate results
    cash_roll_rets = [r["cash_roll_return"] for r in oscillating_results]
    spy_rets = [r["spy_return"] for r in oscillating_results]

    avg_cash_roll = sum(cash_roll_rets) / len(cash_roll_rets)
    avg_spy = sum(spy_rets) / len(spy_rets)

    cash_roll_wins = sum(1 for i in range(len(oscillating_results))
                         if oscillating_results[i]["cash_roll_return"] > oscillating_results[i]["spy_return"])
    spy_wins = len(oscillating_results) - cash_roll_wins

    # Annualized metrics (average monthly return * 12)
    ann_cash_roll = (1 + avg_cash_roll) ** 12 - 1
    ann_spy = (1 + avg_spy) ** 12 - 1

    # Sharpe-like: avg return / std (monthly)
    ...
    cum_spy_series = [1.0]
    for entry in oscillating_results:
    ...
    results = {
        "claim": "#23 - 钱仓滚存震荡市优于持有",
        "regime_definition": {
            "oscillating": f"SPY 6-month return between {OSCILLATE_LOWER:.0%} and {OSCILLATE_UPPER:.0%}",
        ...
        "oscillating_month_count": len(oscillating_results),
        "summary_metrics": {
            "cash_roll": {
            ...
            "cash_roll_outperformance": {
                "avg_monthly_outperformance": round(avg_cash_roll - avg_spy, 6),
                "ann_outperformance": round(ann_cash_roll - ann_spy, 6),
                "win_rate_vs_spy": round(cash_roll_wins / len(oscillating_results), 4),
                "total_periods": len(oscillating_results),
            },
        },
        "claim_supported": avg_cash_roll > avg_spy,
        "monthly_details": oscillating_results[:120],  # limit output size
        "summary": (
            "如果钱仓滚存策略在震荡市的平均月收益 > SPY持有, "
            "则论断成立: 震荡市中钱仓滚存优于持有."
        ),
    }
...
if __name__ == "__main__":
    ...
    print(f"震荡市月数: {results['oscillating_month_count']}")
    print()
    ...
    outperform = sm["cash_roll_outperformance"]
    print(f"钱仓滚存月均跑赢 SPY: {outperform['avg_monthly_outperformance']:+>+9.4%}")
    print(f"钱仓滚存年化跑赢 SPY: {outperform['ann_outperformance']:+>+9.4%}")
    print(f"跑赢概率: {outperform['win_rate_vs_spy']:.1%} ({outperform['total_periods']} 个月)")
